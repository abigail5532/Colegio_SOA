...
FROM alumnos al INNER JOIN aulas au ON al.aula = au.idaula INNER JOIN grados g ON au.grado = g.idgrado 
INNER JOIN niveles n ON g.nivel = n.idniv WHERE al.aula = $idaula ORDER BY nombre_completo ASC");
...
<table border="1">
    ...
    if (mysqli_num_rows($query) > 0) {
        while ($data = mysqli_fetch_assoc($query)) {
            $id = $idCounter++;
            $nombreCompleto = strtoupper(htmlspecialchars($data['nombre_completo'], ENT_QUOTES, 'UTF-8'));
            ?>
            <tr>
                <td><?php echo $id; ?></td>
